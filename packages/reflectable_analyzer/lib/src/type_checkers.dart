...
abstract class TypeChecker {
  ...
  const factory TypeChecker.fromRuntime(Type type, Reflectable reflector) =
      _MirrorTypeChecker;
  ...
  bool isAssignableFrom(Element element) {
    final _isExact = isExactly(element);

    final _isSuperType =
        (element is ClassElement && element.allSupertypes.any(isExactlyType));
    // print('isExactly: ${_isExact}');
    // print('isSuperType: ${_isSuperType}');
    return _isExact || _isSuperType;
  }
...
class _MirrorTypeChecker extends TypeChecker {
  static Uri _uriOf(TypeMirror mirror) {
    final uri = (mirror.owner as LibraryMirror).uri;
    // print('URI: $uri');
    final normalizedUri = normalizeUrl(uri);
    // print('NORMALIZED URI: $normalizedUri');
    final uriWithFragment = normalizedUri.replace(fragment: mirror.simpleName);
    // print('FRAG: $uriWithFragment');
  ...
  const _MirrorTypeChecker(this._type, this.reflector) : super._();

  TypeChecker get _computed {
    final comp = _cache[this] ??=
        TypeChecker.fromUrl(_uriOf(reflector.reflectType(_type)));
    return comp;
  }
...
class _UriTypeChecker extends TypeChecker {
  ...
  bool hasSameUrl(dynamic url) {
    final typeUrl = uri.toString();
    final otherUrl =
        (url is String ? url : normalizeUrl(url as Uri).toString());
    // print('OTHER URI: $url');
    // print('OTHER NORMALIZED URI: $otherUrl');
    // print('CHECKER URI: $typeUrl');
    return typeUrl == otherUrl;
  }
