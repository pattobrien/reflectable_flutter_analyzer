...
// copied from package:source_gen
...
Uri normalizeUrl(Uri url) {
  switch (url.scheme) {
    ...
    case 'reflectable':
      return normalizeReflectableUrl(url);
    default:
      return url;
  }
}
...
Uri normalizeReflectableUrl(Uri url) {
  // print('url: $url');
  if (url.pathSegments[0].startsWith('dart')) {
    final segs = url.pathSegments[0].split('.');
    return Uri.parse('dart:${segs[1]}');
  }
